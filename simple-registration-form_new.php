  ...
  if (is_array($uploadfiles)) {
    ...
    foreach ($uploadfiles['name'] as $key => $value) {
      ...
      if ($uploadfiles['error'][$key] == 0) {
         ...
         */
        $i = 0;
        while ( file_exists( $upload_dir['wp-content/uploads'] .'/' . $filename ) ) {
          $filename = $filetitle . '_' . $i . '.' . $filetype['ext'];
          $i++;
        }
        $filedest = $upload_dir['wp-content/uploads'] . '/' . $filename;

        /**
         * Check write permissions
         */
        if ( !is_writeable( $upload_dir['wp-content/uploads'] ) ) {
        ...
        $attach_data = wp_generate_attachment_metadata( $attachment, $filedest );
		...
		$status = $wpdb->insert(cart,array('p_name'=>$productname,'p_img'=>$attachment));

		$img = "insert into cart('p_name','p_img') values($productname,$attachment)";
...
function show_cart()
{
		...
		foreach($cart_data as $row)
		{
			...
			echo "<input type='button' value=\"add\"></button>"."<br>";
